    ...
    public override async Task<Empty> EmptyCart(EmptyCartRequest request, ServerCallContext context)
    {
        ...
        try
        {
            if (await _featureFlagHelper.GetBooleanValueAsync("cartFailure", false))
            {
                ...
                catch (RpcException badStoreEx)
                {
                    _logger.LogWarning(badStoreEx, "cartFailure flag triggered bad cart store failure for userId={UserId}; falling back to primary cart store", request.UserId);
                    Activity.Current?.AddEvent(new ActivityEvent("cart_failure_flag_fallback"));
