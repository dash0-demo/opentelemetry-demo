    ...
    public override async Task<Empty> EmptyCart(EmptyCartRequest request, ServerCallContext context)
    {
        ...
        try
        {
            if (await _featureFlagHelper.GetBooleanValueAsync("cartFailure", false))
            {
                ...
                {
                    // cartFailure flag is active but the bad store failed as expected.
                    // Fall back to the healthy store so checkout can complete successfully.
                    _logger.LogWarning(ex, "cartFailure flag triggered a store error; falling back to healthy cart store for user {UserId}", request.UserId);
                    activity?.SetTag("cart.failure_flag_fallback", true);
